...
class shopify_refund_wizard(models.TransientModel):
    ...
    def refund_in_shopify(self):    
        ...
        for refund in refunds:
            ...
            picking_total_qty = 0.0
            sale_total_qty = 0.0
            shipping={}
            full_refund=0.0
            for order in orders:
                picking_id = order.mapped('picking_ids').filtered(lambda picking: picking.state == 'done')
                if not picking_id:
                    break
                for move_line in picking_id.move_lines:
                    if move_line.product_id.type == 'service':
                        continue
                    picking_total_qty +=move_line.quantity_done
                order_line = picking_id.sale_id.order_line
                for order_line_id in order_line:
                    if order_line_id.product_id.type == 'service':
                        continue
                    sale_total_qty += order_line_id.product_uom_qty
                if picking_total_qty == sale_total_qty:
                    full_refund = picking_id.sale_id.amount_total
            ...
            for order in orders:
                if not_process:
                    continue
                refund.shopify_instance_id.connect_in_shopify()
                refund_amount = full_refund or refund.amount_total
